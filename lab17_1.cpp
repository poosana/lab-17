...
void getCommand(string &command , string &search){
    string text;
    cout<<"Please input your command: ";
    getline(cin,text);
    int lok = text.find_first_of(" ");
    command = text.substr(0,lok);
    search = text.substr(lok+1);

}

void searchName(vector<string> &names , vector<int> &scores, vector<char> &grades , string search){
    int n = names.size();
    int bo = 0;
    cout <<"---------------------------------\n";
    for (int i = 0; i < n; i++)
    {
        if(toUpperStr(names[i])==search){
...
void searchGrade(vector<string> &names , vector<int> &scores , vector<char> &grades, string search ){
    cout <<"---------------------------------\n";
    int n = names.size();
    char key1[100];
    int bo = 0;
    strcpy(key1,search.c_str());
    for (int i = 0; i < n; i++){
        if (grades[i]==*key1){
